"""DLA simulation using Numba-JIT SOR.

Same DLA as a2_1_dla.py but uses the Numba-compiled SOR variant which
JIT-compiles the inner double loop for ~50-100× speedup over pure Python.
"""
...
# ── Warm up Numba JIT (compile once on a tiny grid) ─────────────────────────
print("Warming up Numba JIT compiler...")
t_jit = time.perf_counter()
warmup_numba()
print(f"JIT warmup done in {time.perf_counter() - t_jit:.1f} s")
...
print(f"DLA (Numba SOR): N={N}, n_steps={N_STEPS}, η={ETA}, ω={OMEGA:.4f}")
...
print(f"Wall time: {elapsed:.1f} s  (excluding JIT warmup)")
...
ax.set_title("Numba SOR iterations per step\n(warm-start)")
ax.grid(True, alpha=0.3)

fig.suptitle(f"PDE-based DLA — Numba SOR ({N}×{N}, {elapsed:.1f} s)")
plt.tight_layout()

out_dir = Path(__file__).parent.parent / "images" / "figures"
out_dir.mkdir(parents=True, exist_ok=True)
fname = out_dir / "a2_1_dla_numba.png"
